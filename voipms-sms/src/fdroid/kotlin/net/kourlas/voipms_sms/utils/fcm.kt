 ...
 *
 * Requires the relay URL to be configured (Settings -> Notifications -> Push).
 * Selects the current/default distributor (e.g. ntfy), then hands off to
 * [UnifiedPushRegistrationWorker] which fetches the relay's VAPID key and
 * registers one UnifiedPush instance per DID. Endpoints arrive asynchronously
 * in [net.kourlas.voipms_sms.notifications.VoipmsPushService.onNewEndpoint].
 */
